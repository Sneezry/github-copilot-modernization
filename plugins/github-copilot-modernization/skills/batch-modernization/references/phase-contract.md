...
The batch control plane owns configuration, preflight, lease, state, event, summary, and result verification. A future phase agent owns one execution unit, one phase, and one attempt.
...
- Takeover compare-and-swap rotates ownership but is read-only in Stage 1A.
- A read-only takeover cannot schedule or mutate until worker fencing is implemented.

## Deferred Behavior

Stage 1A does not dispatch agents, resume work, retry attempts, process `NeedsInput`, or mutate source workspaces. Those capabilities require their later-stage gates.